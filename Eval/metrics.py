...
class IoU(object):
    ...
    def _cal_iou(self, pred: np.ndarray, gt: np.ndarray):
        ...
        intersection = np.diag(confusion_matrix)  # Intersection
        union = np.sum(confusion_matrix, axis=1) + np.sum(confusion_matrix, axis=0) - np.diag(confusion_matrix)  # Union
        IoU = intersection / union  # Intersection over Union (IoU)
        return IoU
