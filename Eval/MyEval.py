    ...
    for _data_name in opt.data_lst:
        ...
        with open(filename, 'w+') as file_to_write:
            ...
            for _model_name in opt.model_lst:
                ...
                tb.add_row([_data_name, _model_name, sm.round(4), wfm.round(4), mae.round(4), em['adp'].round(4), em['curve'].mean().round(4), em['curve'].max().round(4), fm['adp'].round(4), fm['curve'].mean().round(4), fm['curve'].max().round(4)])
            print(tb)
...
def eval_super_class(opt):
    ...
    for _model_name in opt.model_lst:
        for _super_cls in _super_cls_lst:
            ...
            tb.add_row([_super_cls, _model_name, sm.round(4), wfm.round(4), mae.round(4), em['adp'].round(4),
                        em['curve'].mean().round(4), em['curve'].max().round(4), fm['adp'].round(4),
                        fm['curve'].mean().round(4), fm['curve'].max().round(4)])
        print(tb)
...
def eval_sub_class(opt):
    ...
    for _model_name in opt.model_lst:
        for _sub_cls in _sub_cls_lst:
            ...
            tb.add_row([_sub_cls, _model_name, sm.round(4), wfm.round(4), mae.round(4), em['adp'].round(4),
                        em['curve'].mean().round(4), em['curve'].max().round(4), fm['adp'].round(4),
                        fm['curve'].mean().round(4), fm['curve'].max().round(4)])
        print(tb)
...
if __name__ == '__main__':
    ...
    txt_save_path = './Eval/eval_txt/{}/'.format(opt.txt_name)
